            ...
            <div className="venue-photo-group" key={copy} aria-hidden={copy === 1 ? true : undefined}>
              {[1, 2, 3, 4, 5].map(n => (
                <button key={n} type="button" tabIndex={copy === 1 ? -1 : 0}
                  onClick={() => onImage({ src: prefix + '-' + n + '.jpg', alt: title + ' · ' + n })}
...
function Home({t,w,lang}) {
  return <>
    ...
    <section className="section wrap introduction"><div className="intro-images"><Photo src="fac-pool.jpg" alt={t.facPool} /><Photo src="s64-1.jpg" alt="Sky Bar 64" /></div><div className="intro-copy"><p className="eyebrow">{w.welcome}</p><h2>{w.intro}</h2><span className="gold-rule" /><p className="muted">{w.introBody}</p></div></section>
