import VirtualTour, { TourTeaser } from './VirtualTour.jsx';
...
function GalleryStrip({ prefix, title, onImage, w }) {
  ...
  return (
    <div className="venue-photo-ribbon" data-paused={paused}>
      <div className="venue-photo-controls">
        <button className="venue-photo-toggle" type="button" onClick={() => setPaused(value => !value)} aria-controls={'photos-' + prefix}>
          {paused ? w.resumePhotos : w.pausePhotos}
            ...
            <div className="venue-photo-group" key={copy} aria-hidden={copy === 1 ? true : undefined}>
              {[1, 2, 3, 4, 5].map(n => (
                <button key={n} type="button" tabIndex={copy === 1 ? -1 : 0}
                  onClick={() => { setPaused(true); onImage({ src: prefix + '-' + n + '.jpg', alt: title + ' · ' + n }); }}
                  aria-label={w.image + ': ' + title + ' ' + n}>
                  <Photo src={prefix + '-' + n + '.jpg'} alt={copy === 1 ? '' : title + ' · ' + n} />
                </button>
              ))}
            </div>
          ))}
...
function Header({ page, lang, setLang, t, w }) {
  ...
  const links = [['home',t.navHome],['stay',w.stay],['facilities',t.navFacilities],['dining',t.navDining],['airportToHotel',w.journey],['virtualTour',w.tour],['contact',t.navContact]];
...
function Home({t,w,lang}) {
  return <>
    <section className="home-hero"><Photo src="bg-home.jpg" alt="Suvarnabhumi Ville Hotel" eager /><div className="hero-shade" /><div className="home-hero-content wrap"><p className="eyebrow pale">THE PERFECT PLACE WITH A PERFECT VIEW</p><h1>{w.hero}</h1><p>{w.heroBody}</p><div className="actions"><External href={BOOK}>{t.navBook}</External><a href="#stay" className="button outline-light">{w.stay}<ArrowRight size={17} /></a></div></div><a className="hero-tour-card" href="#virtualTour"><Photo src="fac-pool.jpg" alt="" /><span><strong>360°</strong><small>{w.tour}</small></span><ArrowUpRight size={22} aria-hidden="true" /></a><span className="hero-side" aria-hidden="true">A PAUSE BETWEEN DESTINATIONS</span></section>
    ...
    <TourTeaser lang={lang}/><FAQ t={t} w={w} />
  </>;
}
function Stay({t,w,lang,onImage}) {
  return <><Banner title={w.stayTitle} subtitle={w.stayBody} image="bg-home.jpg" w={w} /><RoomCatalogue lang={lang} onImage={onImage}/><TourTeaser lang={lang}/><section className="pearl section"><div className="wrap split"><div><p className="eyebrow">THOUGHTFUL DETAILS</p><h2>{w.request}</h2><p className="muted">{w.requestBody}</p><External href={LINE} className="text-link">{w.chat}</External></div><div className="stay-perks">{[[Plane,w.journey,t.service24h,'airportToHotel'],[Waves,t.navFacilities,w.pool,'facilities'],[Coffee,t.navDining,w.food,'dining']].map(([Icon,title,detail,href])=><a href={'#'+href} key={href}><Icon /><div><h3>{title}</h3><p>{detail}</p></div><ArrowUpRight size={17} /></a>)}</div></div></section><FAQ t={t} w={w} /></>;
}
...
export default function App() {
  ...
  useEffect(()=>{const titles={home:t.navHome,stay:w.stay,airportToHotel:t.navAirToHotel,hotelToAirport:t.navHotelToAir,facilities:t.navFacilities,dining:t.navDining,virtualTour:w.tour,contact:t.navContact};document.title=titles[page]+' | Suvarnabhumi Ville Airport Hotel';},[page,t,w]);
  const content={home:<Home t={t} w={w} lang={lang}/>,stay:<Stay t={t} w={w} lang={lang} onImage={setImage}/>,airportToHotel:<Transfer t={t} w={w} onImage={setImage}/>,hotelToAirport:<Transfer departure t={t} w={w} onImage={setImage}/>,facilities:<Facilities t={t} w={w} onImage={setImage}/>,dining:<Dining t={t} w={w} onImage={setImage}/>,virtualTour:<VirtualTour lang={lang}/>,contact:<Contact t={t} w={w}/>};
