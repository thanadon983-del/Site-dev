...
const BOOK = 'https://www.suvarnabhumiville.com/accommodation/room/room-rate';
...
function Photo({ src, alt, className = '', eager = false }) {
  return <img src={'./' + src} alt={alt} className={className} loading={eager ? 'eager' : 'lazy'} fetchPriority={eager ? 'high' : 'auto'} decoding="async" />;
}
...
function Stay({t,w}) {
  return <><Banner page={w.stay} title={w.stayTitle} subtitle={w.stayBody} image="bg-home.jpg" t={t} w={w} /><section className="section wrap split"><div className="stay-image"><Photo src="bg-hotel.jpg" alt="Suvarnabhumi Ville Hotel" /><span className="photo-label">SUVARNABHUMI VILLE AIRPORT HOTEL</span></div><div><p className="eyebrow">YOUR STAY</p><h2>{w.stay}</h2><p className="muted">{w.stayBody}</p><div className="booking-panel"><CalendarDays size={26} /><h3>{w.official}</h3><p>{w.roomNote}</p><External href={BOOK}>{w.roomLink}</External></div></div></section><section className="pearl section"><div className="wrap split"><div><p className="eyebrow">THOUGHTFUL DETAILS</p><h2>{w.request}</h2><p className="muted">{w.requestBody}</p><External href={LINE} className="text-link">{w.chat}</External></div><div className="stay-perks">{[[Plane,w.journey,t.service24h,'airportToHotel'],[Waves,t.navFacilities,w.pool,'facilities'],[Coffee,t.navDining,w.food,'dining']].map(([Icon,title,detail,href])=><a href={'#'+href} key={href}><Icon /><div><h3>{title}</h3><p>{detail}</p></div><ArrowUpRight size={17} /></a>)}</div></div></section><FAQ t={t} w={w} /></>;
}
...
export default function App() {
  ...
  const content={home:<Home t={t} w={w}/>,stay:<Stay t={t} w={w}/>,airportToHotel:<Transfer t={t} w={w} onImage={setImage}/>,hotelToAirport:<Transfer departure t={t} w={w} onImage={setImage}/>,facilities:<Facilities t={t} w={w} onImage={setImage}/>,dining:<Dining t={t} w={w} onImage={setImage}/>,gallery:<Gallery t={t} w={w} onImage={setImage}/>,contact:<Contact t={t} w={w}/>};
