...
function Home({t,w,lang}) {
  return <>
    <section className="home-hero"><Photo src="bg-home.webp" alt="Suvarnabhumi Ville Hotel" eager /><div className="hero-shade" /><div className="home-hero-content wrap"><p className="eyebrow pale">THE PERFECT PLACE WITH A PERFECT VIEW</p><h1>{w.hero}</h1><p>{w.heroBody}</p><div className="actions"><External href={BOOK}>{t.navBook}</External><a href="#stay" className="button outline-light">{w.stay}<ArrowRight size={17} /></a></div></div><a className="hero-tour-card" href="#virtualTour"><Photo src="fac-pool.webp" alt="" /><span><strong>360°</strong><small>{w.tour}</small></span><ArrowUpRight size={22} aria-hidden="true" /></a><span className="hero-side" aria-hidden="true">A PAUSE BETWEEN DESTINATIONS</span></section>
    ...
    <section className="section wrap introduction"><div className="intro-images"><Photo src="fac-pool.webp" alt={t.facPool} /><Photo src="home-dining.webp" alt="Sky Bar 64" /></div><div className="intro-copy"><p className="eyebrow">{w.welcome}</p><h2>{w.intro}</h2><span className="gold-rule" /><p className="muted">{w.introBody}</p></div></section>
    <section className="section pearl"><div className="wrap"><div className="section-heading"><div><p className="eyebrow">THE VILLE COLLECTION</p><h2>{w.experiences}</h2></div><a href="#facilities" className="text-link">{w.explore}<ArrowUpRight size={18} /></a></div><div className="experience-grid">{[['facilities','fac-pool.webp',t.navFacilities,w.pool],['dining','home-skybar.webp',t.navDining,w.food],['airportToHotel','bg-hotel.webp',w.journey,t.service24h]].map(([id,img,label,desc],i)=><a className="experience-card" key={id} href={'#'+id}><Photo src={img} alt={label} /><span className="card-number">0{i+1}</span><div><p className="eyebrow pale">{desc}</p><h3>{label}</h3><ArrowUpRight aria-hidden="true" /></div></a>)}</div></div></section>
    <section className="dining-teaser"><Photo src="home-skybar.webp" alt="Sky Bar 64" /><div className="wrap"><div><p className="eyebrow pale">AN EVENING AT THE VILLE</p><h2>Sky Bar 64</h2><p>{t.s64Desc}</p><a href="#dining" className="button outline-light">{t.navDining}<ArrowRight size={17} /></a></div></div></section>
    <TourTeaser lang={lang}/><FAQ t={t} w={w} />
  </>;
}
function Stay({t,w,lang,onImage}) {
  return <><Banner title={w.stayTitle} subtitle={w.stayBody} image="bg-rooms.webp" w={w} /><RoomCatalogue lang={lang} onImage={onImage}/><TourTeaser lang={lang}/><section className="pearl section"><div className="wrap split"><div><p className="eyebrow">THOUGHTFUL DETAILS</p><h2>{w.request}</h2><p className="muted">{w.requestBody}</p><External href={LINE} className="text-link">{w.chat}</External></div><div className="stay-perks">{[[Plane,w.journey,t.service24h,'airportToHotel'],[Waves,t.navFacilities,w.pool,'facilities'],[Coffee,t.navDining,w.food,'dining']].map(([Icon,title,detail,href])=><a href={'#'+href} key={href}><Icon /><div><h3>{title}</h3><p>{detail}</p></div><ArrowUpRight size={17} /></a>)}</div></div></section><FAQ t={t} w={w} /></>;
}
...
function Footer({t,w}) {
  return <><footer className="footer"><div className="wrap footer-grid"><div><a href="#home" className="footer-brand"><img className="hotel-logo" src="./logo-large.png" alt="Suvarnabhumi Ville" loading="lazy" /></a><p>{t.slogan}</p><address>{t.addressDesc}</address></div><div><h3>{w.explore}</h3>{[['stay',w.stay],['promotions',w.promotions],['facilities',t.navFacilities],['dining',t.navDining],['virtualTour',w.tour]].map(([id,label])=><a href={'#'+id} key={id}>{label}</a>)}</div><div><h3>{w.journey}</h3><a href="#airportToHotel">{t.navAirToHotel}</a><a href="#hotelToAirport">{t.navHotelToAir}</a><a href="#contact">{t.navContact}</a><a href="tel:+66982673888">+66 (0) 98 267 3888</a><a href="https://mail.google.com/mail/?view=cm&fs=1&to=front%40suvarnabhumiville.com">front@suvarnabhumiville.com</a></div><div><h3>{w.social}</h3>{[['Facebook','https://www.facebook.com/suvarnabhumi.ville.2025'],['Instagram','https://www.instagram.com/suvarnabhumiville/'],['TikTok','https://www.tiktok.com/@suvarnabhumivilles64'],['LINE',LINE],['WhatsApp',WHATSAPP],['WeChat',WECHAT_QR]].map(([name,url])=><External key={name} href={url} className="footer-social">{name}</External>)}</div></div><div className="wrap footer-bottom"><span>© {new Date().getFullYear()} Suvarnabhumi Ville Hotel. {t.allRightsReserved}</span><a href="#home">SUVARNABHUMI VILLE AIRPORT HOTEL</a></div></footer><div className="mobile-booking"><a href="tel:+66982673888"><Phone size={18}/>{w.call}</a><External href={BOOK}>{t.navBook}</External></div></>;
}
